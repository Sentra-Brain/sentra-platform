...
export const regenerateTitle = createAsyncThunk(
  'conversations/regenerateTitle',
  async (id: string) => {
    return await conversationService.generateTitle(id)
  }
)
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(regenerateTitle.fulfilled, (state, action) => {
        if (action.payload.title && action.payload.conversation_id) {
          const conversation = state.conversations.find(c => c.id === action.payload.conversation_id)
          if (conversation) conversation.title = action.payload.title
        }
      })
  },
})

export const { selectConversation, clearConversation, updateConversationTitle } = conversationsSlice.actions
