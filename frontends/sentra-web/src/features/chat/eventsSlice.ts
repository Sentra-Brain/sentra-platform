...
/**
 * Minimal subset of AG-UI events we actually care about in the UI.
 * Extend this union if you later need more event kinds.
 */
  ...
  for (const event of events) {
    switch (event.type) {
      case EventType.TEXT_MESSAGE_START: {
        // TextMessageStartEvent has messageId + optional role
        ensureMessageEntry(event.messageId, event.role);
        break;
      }

      case EventType.TEXT_MESSAGE_CONTENT: {
        // TextMessageContentEvent has messageId + delta
