...
def register_toolbelt(mcp: FastMCP) -> None:
    ...
    @mcp.tool("text_truncate")
    def text_truncate(text: str, max: int = 8192) -> str:  # noqa: A002
        return text[:max]


    @mcp.tool("text_extract")
    def text_extract(text: str, regex: str, flags: Sequence[str] = ()) -> list[str]:
        comp = re.compile(regex, _regex_flags(flags))
        return comp.findall(text)

    @mcp.tool("text_replace")
    def text_replace(
        text: str,
        regex: str,
        repl: str,
    ...
    @mcp.tool("fs_read_text")
    def fs_read_text(path: str) -> str:
        p = _resolve_fs(path)
        return p.read_text()


    @mcp.tool("fs_write_text")
