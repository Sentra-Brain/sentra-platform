...
class ConversationEngine:
    ...
    async def run(self, request: ConversationRequest) -> AsyncGenerator[ConversationEvent, None]:
        ...
        if request.context_source_ids or request.context_document_ids:
            # Emit step_start for RAG retrieval
        ...
        if request.context_source_ids or request.context_document_ids:
            # Emit step_end for RAG retrieval with metadata
        ...
        async for delta in self._vllm_stream(payload):
            buffer += delta
            # Emit message_delta for each token
            yield ConversationEvent(
                type="message_delta",
                content=delta
            )
        ...
        if not buffer.strip():
            ...
            # Emit message_final even for empty response
            yield ConversationEvent(
                type="message_final",
                content="(No response)"
            )
        ...
        # Emit message_final with complete content
        yield ConversationEvent(
            type="message_final",
            content=buffer
        )
